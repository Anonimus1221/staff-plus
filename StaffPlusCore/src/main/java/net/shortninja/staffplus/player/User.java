...
public class User implements IUser {
     ...
     * 
     * craftPlayerClass = Class.forName("org.bukkit.craftbukkit." + version +
     * ".entity.CraftPlayer");
     * entityPlayerClass = Class.forName("net.minecraft.server." + version +
     * ".EntityPlayer");
     * playerConnectionClass = Class.forName("net.minecraft.server." + version +
     * ".PlayerConnection");
    ...
    public User(UUID uuid, String name, short glassColor, List<IReport> reports, List<IWarning> warnings,
            List<String> playerNotes, Map<AlertType, Boolean> alertOptions) {
        this.uuid = uuid;
        this.name = name;
    ...
    public void addReport(IReport report) {
        if (options.storageType.equalsIgnoreCase("flatfile"))
            ...
            try (Connection sql = MySQLConnection.getConnection();
                    PreparedStatement insert = sql
                            .prepareStatement("INSERT INTO sp_reports(Reason, Reporter_UUID, Player_UUID) " +
                                    "VALUES(?, ?, ?);");) {
    ...
    public void addWarning(IWarning warning) {
        if (options.storageType.equalsIgnoreCase("flatfile"))
            ...
            try (Connection sql = MySQLConnection.getConnection();
                    PreparedStatement insert = sql
                            .prepareStatement("INSERT INTO sp_warnings(Reason, Warner_UUID, Player_UUID) " +
                                    "VALUES(? ,?, ?);");) {
