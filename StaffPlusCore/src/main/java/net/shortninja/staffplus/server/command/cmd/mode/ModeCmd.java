...
public class ModeCmd extends BukkitCommand {
        ...
        if (args.length >= 2 && permission.isOp(sender)) {
            ...
            if (targetPlayer != null) {
                if (option.equalsIgnoreCase("enable")) {
                    ...
                    message.send(sender, messages.invalidArguments.replace("%usage%", getName() + " &7" + getUsage()),
    ...
    private void toggleMode(Player player) {
        if (StaffPlus.get().modeCoordinator.isInMode(player.getUniqueId())) {
            ...
            message.send(player, "&cStaff mode disabled", "Staff+");
        } else {
            StaffPlus.get().modeCoordinator.addMode(player);
            message.send(player, "&aStaff mode enabled! Check your inventory for gadgets.", "Staff+");
        }
    }
}