...
public class PlayerCommandPreprocess implements Listener {
    ...
    public void onCommand(PlayerCommandPreprocessEvent event) {
        ...
        if (options.blockedCommands.contains(command) && permission.hasOnly(player, options.permissionBlock)) {
            ...
            event.setCancelled(true);
        } else if (freezeHandler.isFrozen(uuid) && (!options.modeFreezeChat
                || (freezeHandler.isLoggedOut(uuid)) && !command.startsWith("/" + options.commandLogin))) {
            ...
            if (baseCmd.getPermissions().isEmpty()) {
                ...
                for (String permission : baseCmd.getPermissions()) {
                    if (this.permission.has(player, permission)) {
                        message.send(player,
                                "&b/" + baseCmd.getMatch() + " &8: " + baseCmd.getDescription().toLowerCase(), "");
                        count++;
                        break;
                    }
                }
